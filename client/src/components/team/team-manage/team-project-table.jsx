...
import CreateProjectSheet from "@/components/team/actions/create-project-sheet";
import {
  Table,
  TableBody,
  TableCell,
...
import { getColumns } from "@/components/team/team-manage/team-project-column";
...
export default function TeamProjectTable() {
  ...
  const [data, setData] = useState(projects[selectedTeam.id]);
  ...
  const editable = ["owner", "admin"].includes(selectedTeam.role);

  useEffect(() => {
    setData(projects[selectedTeam.id]);
  ...
  return (
    <div className="flex flex-col gap-2 py-4 bg-ghost-white border-1 rounded-md">
      ...
      <CreateProjectSheet
        open={isCreateProjectSheetOpen}
        onOpenChange={setIsCreateProjectSheetOpen}
      />
    </div>
  );
}
